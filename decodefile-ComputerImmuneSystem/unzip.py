...
# 解压gz，事实上就是读出当中的单一文件
def un_gz(file_name_dest, file_name_src):
    """ungz zip file"""
    #获取文件的名称，去掉
    g_file = gzip.GzipFile(file_name_src)
    #创建gzip对象
    open(file_name_dest, "wb+").write(g_file.read())
...
def un_tar(file_name_dest, file_name_src):
       # untar zip file"""
    tar = tarfile.open(file_name_src)
    names = tar.getnames()
    if os.path.isdir(file_name_dest):
        pass
    else:
        os.mkdir(file_name_dest)
    #因为解压后是很多文件，预先建立同名目录
    for name in names:
        tar.extract(name, file_name_dest + "/")
    tar.close()
